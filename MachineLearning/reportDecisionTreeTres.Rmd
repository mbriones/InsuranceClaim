...
dataMB <- data.frame(DANO = "Dermatologico", COMUNIDAD_AUTONOMA = "MADRID")
predictMB <- max(predict(fit, dataMB))
classMB <- as.character(predict(fit, dataMB, type = "class"))

predict(fit, dataMB, type = "class")
...
datex2$log <- log10(datex2$IMPORTE)
datex2$exp <- 10^(datex2$log)

datex2 <- datex[datex$IMPORTE >= 0, ]

pikachu <- sapply(subset(datex, DANO == "Muerte" & COMUNIDAD_AUTONOMA == "MADRID", 
           select = c(IMPORTE)), mean)
          
blast <- sapply(subset(datex, DANO == "Muerte" & COMUNIDAD_AUTONOMA == "MADRID", 
           select = c(IMPORTE)), sd)
